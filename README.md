...
1 段目が名前と `⌘` の番号、実行状態（`●` 実行中 / `○` 終了、終了コードが
0 でなければ赤）。2 段目が作業ディレクトリ。3 段目はブランチ名で、git の
下にいないときは段そのものを作らない。

名前の後ろの `*` は、親の会話を引き継いでいない分岐であることを示す。
分岐は親の下に字下げして並べる。

ブランチ名は `git` を起動せず `.git/HEAD` を読む。作業ディレクトリが
変わったときと、2 秒以上経ったときだけ読み直す。
